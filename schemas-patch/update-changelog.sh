...
pkg_json="${base}/src/schemas/package.json"
changelog="${base}/debian/changelog"
git_dir="${base}/src/schemas"
...
entry="drumee-patch (${version}) stable; urgency=medium
${bullets}
 -- ${maintainer} <${email}>  ${date_str}"

tmpfile=$(mktemp)

if grep -q "^drumee-patch (${version})" "$changelog"; then
  # Find line number of the existing entry and the next entry
  start=$(grep -n "^drumee-patch (${version})" "$changelog" | head -1 | cut -d: -f1)
  next_line=$(awk "NR > $start && /^drumee-patch \(/ {print NR; exit}" "$changelog")
