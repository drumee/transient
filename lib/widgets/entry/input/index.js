...
class __drumee_entry_input extends LetcBox {
  constructor(...args) {
    ...
    this._on_keyup = this._on_keyup.bind(this);
  ...
  static initClass() {
    ...
    this.prototype.events = {
      'keydown input': '_on_keydown',
      'keydown textarea': '_on_keydown',
      'keyup   input': '_on_keyup',
      'keyup   textarea': '_on_keyup',
      'blur    input': '_on_blur',
      'select  input': '_on_select',
      'submit  form': '_nop',
      click: '_on_click',
  ...
  initialize(opt) {
    ...
    this._comply = compliances(this._require);
    ...
    this.declareHandlers(); //s(part:@, fork:yes)
  }
    ...
    switch (e.key) {
      case _e.Enter:
        if (this.mget(_a.mode) == _a.commit) {
          ...
          this.triggerHandlers(this._status(_e.Enter));
        }
        return true;
      ...
      case 'ArrowDown':
        if (_.isFinite(~~this.getValue())) {
          return this.triggerHandlers(this._status('arrow'));
        }
        break;
      ...
      case _e.Escape:
        ...
        return this.triggerHandlers(this._status(_e.cancel));
