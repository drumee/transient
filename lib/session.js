...
class coreSession extends Logger {
  ...
  async _selectSession() {
    ...
    /** _initHub and _initUser read independent inputs -- authorization() derives
     *  only from cookie/query/host on Input, and _initUser never reads hub state
     *  -- so their DB round trips still overlap here.
     *
     *  They are NOT independent in ORDER, though: _initUser ends in
     *  _assign_user(), which fires START, and START is consumed by Acl, which
     *  resolves the request against the hub's database. The promise is kept on
     *  `this` so _assign_user can await the hub before firing. */
    this._hubReady = this._initHub();
  ...
  async _assign_user(user) {
    ...
    this._started = 1;
    /** START (fired below) is consumed by Acl, which resolves the request
     *  against the hub's database. _initHub runs concurrently with _initUser,
     *  and on its slow path -- an unresolvable vhost, where _default_hub() adds
     *  a SECOND get_hub round trip -- an anonymous _initUser can reach here
     *  first and start the request with no hub. The request then falls into the
     *  public media service and calls mfs_home() against `yp`, which has no
     *  `media` table: "Table 'yp.media' doesn't exist".
     *
     *  Awaiting the hub here restores the documented contract (section 8 of
     *  docs/ARCHITECTURE.md: READY/START "with hub + user context set") while
     *  still letting the two queries overlap. Hub failures stay swallowed,
     *  exactly as _selectSession has always treated them. */
    if (this._hubReady) await this._hubReady.catch(() => { });
