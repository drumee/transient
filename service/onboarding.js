...
class Onboarding extends Entity {
  ...
  */
  ...
  /**
   * Invite team members to one or more workspaces with specific roles.
   *
   * Params:
   *   emails {string[]}  — list of email addresses to invite
   *   workspaces {object[]}  — [{hub_id, privilege}] per workspace
   *
   * Privilege values:
   *   3  = View (read only)
   *   6  = Chat (read + chat)
   *   7  = Edit (read + write + chat)
   *   15 = Admin (full access, short of owner)
   *
   * Flow:
   *   1. Validate all emails and workspace privilege values.
   *   2. For each (email × workspace): generate a unique token,
   *      store in yp.workspace_invitation (etime = now + 7 days).
   *   3. Send one invitation email per invitee listing all workspaces.
   */
  async send_workspace_invites() {
    if (!this.uid) {
      return this.exception.user('User not authenticated.');
    }
 
    const emails     = toArray(this.input.need('emails'));
    const workspaces = toArray(this.input.need('workspaces'));
 
    if (!emails.length) {
      return this.exception.user('No emails provided.');
    }
    if (!workspaces.length) {
      return this.exception.user('No workspaces provided.');
    }
 
    // Validate emails
    const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    const invalidEmails = emails.filter(e => !EMAIL_RE.test(String(e).trim()));
    if (invalidEmails.length) {
      return this.exception.user(`Invalid email address(es): ${invalidEmails.join(', ')}`);
    }
 
    // Validate workspaces
    const VALID_PRIVILEGES = [3, 6, 7, 15];
    for (const ws of workspaces) {
      if (!ws.hub_id) {
        return this.exception.user('Each workspace entry must have a hub_id.');
      }
      const priv = parseInt(ws.privilege);
      if (!VALID_PRIVILEGES.includes(priv)) {
        return this.exception.user(
          `Invalid privilege value ${ws.privilege}. Must be one of: 3 (View), 6 (Chat), 7 (Edit), 15 (Admin).`
        );
      }
      ws.privilege = priv;
    }
 
    const tpl      = resolve(__dirname, './templates/workspace-invite.html');
    const homepath = this.input.homepath();
    const EXPIRY   = 7 * 24 * 3600;
    let sent = 0;
 
    for (const rawEmail of emails) {
      const email = String(rawEmail).trim();
 
      // Store one invitation row per (email × workspace) with a unique token
      const tokens = [];
      for (const ws of workspaces) {
        try {
          const token = await this.yp.await_func('uniqueId');
          await this.yp.await_query(
            `INSERT IGNORE INTO workspace_invitation
               (token, inviter_id, email, hub_id, privilege, status, ctime, etime)
             VALUES (?, ?, ?, ?, ?, 'pending', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + ?)`,
            token, this.uid, email, ws.hub_id, ws.privilege, EXPIRY
          );
          tokens.push({ hub_id: ws.hub_id, privilege: ws.privilege, token });
        } catch (e) {
          this.warn(
            `[send_workspace_invites] insert failed for ${email}/${ws.hub_id}:`,
            e && e.message
          );
        }
      }
      ...
      // Build accept URL using the first token; server-side processes all pending rows for this email on acceptance.
      const accept_url = `${homepath}#/welcome/join?invite=${encodeURIComponent(tokens[0].token)}&email=${encodeURIComponent(email)}`;
 
      const data = {
        heading:   'You have been invited to collaborate on Drumee',
        hello:     'Hello,',
        message:   'A colleague has invited you to join one or more workspaces on Drumee — a sovereign workspace for files, chat, and collaboration.',
        link:      accept_url,
      ...
      const msg = new Messenger({
        subject:   'You have been invited to a Drumee workspace',
        recipient: email,
        handler:   this.exception.email,
      });
      ...
      try {
        ...
        this.warn(`[send_workspace_invites] email send failed for ${email}:`, e && e.message);
      }
    }
