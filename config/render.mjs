...
function parseYaml(text) {
  ...
  for (let n = 0; n < lines.length; n++) {
    ...
    const m = raw.trimStart().match(/^([\w-]+):\s*(.*)$/);
...
function renderEnv(cfg) {
  ...
  const pairs = {
    ...
    SERVER_TAG: cfg.versions.server,
    UI_TAG: cfg.versions.ui,
    SCHEMAS_TAG: cfg.versions.schemas,
    STATIC_TAG: cfg.versions.static,
...
services:
  ...
  schemas-init:
    image: drumee/schemas:\${SCHEMAS_TAG}
    networks: [drumee]
    depends_on:
      mariadb:
  ...
  schemas-populate:
    image: drumee/schemas-populate:\${SERVER_TAG}
    networks: [drumee]
    depends_on:
      mariadb:
        condition: service_healthy
  ...
  server-pod:
    image: drumee/server-pod:\${SERVER_TAG}
    restart: unless-stopped
    networks: [drumee]
    depends_on:
      mariadb:
...
switch (command) {
  ...
  default:
    die(`unknown command: ${command ?? '(none)'}\n` +
        'usage: render.mjs validate|env|compose|debconf|all [--config FILE] [--out FILE] [--out-dir DIR]');
}
