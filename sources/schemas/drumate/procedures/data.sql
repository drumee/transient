...
BEGIN 
  ...
  -- SELECT count(*)
  -- FROM yp.disk_usage du LEFT JOIN(yp.entity e, yp.hub h) ON hub_id=e.id AND h.id = e.id 
  -- WHERE h.owner_id=_uid INTO _hubs_count;

  -- SELECT IFNULL(yp.disk_usage(_uid),0) INTO _hubs_count;
  -- SELECT JSON_OBJECT('personal', JSON_OBJECT('size', _personal_size)) AS personal;
  -- SELECT JSON_OBJECT('hub', 
  --   JSON_OBJECT('size', _hubs_size, 'count', _hubs_count)
  --   ) AS hub;
END$
