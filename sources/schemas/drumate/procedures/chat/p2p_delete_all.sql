...
DROP PROCEDURE IF EXISTS `p2p_delete_all`$
CREATE PROCEDURE `p2p_delete_all`(
  IN _in JSON
)
BEGIN
  ...
  -- Case 1: message is in MY DB (I sent it, peer_id = _peer_id)
  ...
  IF _rows_found > 0 THEN
    ...
    IF _max_ctime IS NOT NULL THEN
      UPDATE p2p_time SET ref_ctime = _max_ctime, ctime = _max_ctime WHERE peer_id = _peer_id;
    ELSE
      DELETE FROM p2p_time WHERE peer_id = _peer_id;
    END IF;

    -- Cross-DB: refresh receiver's p2p_time
    SELECT db_name FROM yp.entity WHERE id = _peer_id INTO _peer_db;
    IF _peer_db IS NOT NULL THEN
      IF _max_ctime IS NOT NULL THEN
        SET @_sql = CONCAT(
          "UPDATE `", _peer_db, "`.p2p_time ",
          "SET ref_ctime = ?, ctime = ? WHERE peer_id = ?"
        );
        PREPARE _stmt FROM @_sql;
        EXECUTE _stmt USING _max_ctime, _max_ctime, _uid;
        DEALLOCATE PREPARE _stmt;
      ELSE
        SET @_sql = CONCAT("DELETE FROM `", _peer_db, "`.p2p_time WHERE peer_id = ?");
        PREPARE _stmt FROM @_sql;
  ...
  -- Case 2: message is in PEER's DB (peer sent it, peer_id = _uid in their DB)
