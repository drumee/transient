...
BEGIN
  ...
  SELECT entity_id FROM channel 
    WHERE message_id = _message_id INTO _entity_id;
  ...
  WHILE _idx_node < JSON_LENGTH(_messages) DO 
    ...
    WHILE _idx_attachment < JSON_LENGTH(_attachment) DO 
      ...
      SET @st = CONCAT("DELETE  FROM ", @hub_db_name, ".media  WHERE id =", QUOTE(_nid));
      PREPARE stmt FROM @st;
      EXECUTE stmt ;
      DEALLOCATE PREPARE stmt;
  ...
  IF _entity_db IS NOT NULL THEN
    ...
    SET @st = CONCAT(
      'SELECT max(sys_id) FROM ', _entity_db ,'.channel  
    ...
    SET @st = CONCAT(
      ...
      SELECT c.entity_id, c.sys_id,c.message, c.ctime FROM ', 
      _entity_db ,'.channel c WHERE sys_id =  @_max_sys_id'
    );
    PREPARE stamt FROM @st;
    EXECUTE stamt ;
    DEALLOCATE PREPARE stamt;
    ...
    SET @st = CONCAT(
      'INSERT INTO _last_node
      SELECT ?, ?, LEFT(message, 100), attachment , 0, ctime FROM ', 
      _entity_db ,'.channel  WHERE sys_id = @_max_sys_id'
    );
    PREPARE stamt FROM @st;
