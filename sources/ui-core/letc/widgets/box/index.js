...
const { isNumeric, postService, fetchService } = require("@drumee/ui-essentials")
const validChild = function (e) { return e && e.kind };

/** 
 * Walk in to Object model to create a object in the data 
 */
...
/**
 * Walk through the tree to collect data, set error if any
 * @param {*} c 
 * @param {*} target 
 * @returns Object
 */
function validateData(c, target) {
  const key = _a.formItem;
  ...
  // validateData in to array of data mode eg: array of phone numbers  
  if (name && (c.mget('dataType') === _a.array)) {
    data[name] = [];
    c.children.each(k => {
      let d = validateData(k, target);
      if (d && !_.isEmpty(d)) {
        data[name].push(d);
      }
    });
...
class __drumee_box extends Marionette.CollectionView {
  ...
  */
  ...
  getData(key) {
    ...
    let collectedData = {};
    const rname = this.mget(key);
    if (rname) {
      collectedData[rname] = this.mget(_a.value);
    }
    if (this.mget(_a.formItem) && this.mget(_a.state)) {
      collectedData['name'] = this.mget(_a.formItem);
      collectedData['value'] = this.mget(_a.value) || this.mget(_a.state);
      collectedData[this.mget(_a.formItem)] = this.mget(_a.value) || this.mget(_a.state);
    }

    if (this.isEmpty()) {
      this.mset(collectedData);
      return collectedData;
    }


    if (rname && ((this.mget('dataType') === _a.array) || (this.mget('dataType') === _a.object))) {
      return collectData(this);
    }
    ...
    if (this.children) {
      this.children.each(k => {
        let d = collectData(k);
        if (d && !_.isEmpty(d))
          if (_.isEmpty(collectedData))
            collectedData = d
          else
            collectedData = { ...collectedData, ...d }
      });
    }
    return collectedData;
  }
  ...
  validateData() {
    ...
    let data = validateData(this, this);
    this.formError = data;
