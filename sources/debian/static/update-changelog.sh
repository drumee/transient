...
pkg_json="${base}/src/static/package.json"
changelog="${base}/debian/changelog"
git_dir="${base}/src/static"
...
entry="drumee-static (${version}) stable; urgency=medium
${bullets}
 -- ${maintainer} <${email}>  ${date_str}"

tmpfile=$(mktemp)

if grep -q "^drumee-static (${version})" "$changelog"; then
  # Find line number of the existing entry and the next entry
  start=$(grep -n "^drumee-static (${version})" "$changelog" | head -1 | cut -d: -f1)
  next_line=$(awk "NR > $start && /^drumee-static \(/ {print NR; exit}" "$changelog")
