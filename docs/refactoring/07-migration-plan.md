...
| 2 | Normalize descriptor without loader changes | Build conformance fixtures from sandbox-server/UI, split Loby families, signin and marketplace office integration; use onboarding-server only as legacy migration evidence; adapt ACL JSON, UI `index.json`, seeds and Debian inputs without declaring any live repository canonical | High; stable vocabulary grounded in multiple real patterns |
