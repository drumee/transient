...
must not acquire a fabricated lifetime. A partly upgraded nullable `ctime`
column is handled the same way for `ctime IS NULL`: those unknown-age OTAKs are
deleted, never stamped with the upgrade time. The migration repairs `cookie.uid
IS NULL` to the provisioned, verified `nobody_id` before tightening it to `NOT
...
schema, introduces a representative nullable-`ctime` OTAK, and applies current
schema twice more. It checks the upgraded columns/data and runs the complete
current MariaDB/Redis/Nginx/Chrome Phase 4.4 regression against that upgraded
database. `scripts/test-env/kernel/phase4.4-validation.sh` is the standard
Phase 4.4 entry point and runs both the clean and upgrade modes.
...
the DOM, WebSocket URL, events, or return data. `setSessionAuthorization()`
remains a write-only compatibility boundary, but normal lifecycle updates do
not require an application to call it: the transport privately consumes the
source-issued `regsid` response hand-off described below. Same-origin browser
continuations retain the server-issued HttpOnly cookie without exposing its
value to JavaScript. The client sends the unchanged `x-param-keysel: regsid` /
`x-param-regsid` form after the private transport has received that hand-off.
...
The source assigns distinct responsibilities; the frontend is not a semantic
session owner.

| Source component | Observed responsibility |
| --- | --- |
| `server-core/lib/input.js::_authorization` | selects/creates the server-side active session ID. |
| `server-core/lib/output.js::cookie` | writes both the HttpOnly `Set-Cookie` and the existing response header named by `keysel` (`regsid`). |
| `server-{core,team}` page/bootstrap code and `client/templates/scripts.tpl` | publish `keysel` in `bootstrap()`, never `sid`; they invoke `output.setAuthorization(auth)`. |
| `ui-essentials/socket/utils.js::makeHeaders` | emits the historical `x-param` pair from `bootstrap().sid` or the private `Sessions` map. |
| `ui-essentials/socket/utils.js::setAuthorization` | write-only map operation. A full pinned-frontend/history search found no frontend caller that reads a response and writes this map. |

Thus login keeps the current server session (`Session.signin()` obtains
`this.sid()` and refreshes authorization); it does not independently rotate a
SID in the pinned frontend. Logout clears the server cookie, and a subsequent
page/service lifecycle obtains a new backend session. Replacement is a
server-side event: whenever it emits the historical selected-session response
header, the private transport replaces A with B before its next request.

The target completes this existing server-to-transport loop without creating
storage, a public event, a new request field, or a client-generated ID. For an
allowlisted cross-site response, CORS exposes the already historical `regsid`
header to the transport; it is consumed before the service envelope is
returned to any Widget. This preserves:

```text
no bridge → backend creates regsid → historical regsid response header
→ private transport state → historical x-param bridge → regsid context
...
`Authorization` header, plus CORS visibility of the source-issued `regsid`
response header. The latter is a server session hand-off rather than a request
header; generic `Authorization` remains CORS-allowed but is not a session
parser. Browser preflight is handled by the generic service adapter.
...
Chromium host-resolver rules, starts with neither cookie nor bridge, signs in
through the real service, privately consumes the server-issued historical
response header, and proves that the following `bootstrap.authn` binds an
authenticated socket through the historical headers. It reconnects through a
fresh OTAK, loads Hello and updates the DOM. A separate anonymous cross-site
browser starts without a SID, receives one through `bootstrap.authn`, and
remains denied from `hello.private`. Neither browser fixture injects a SID or
creates a client-side session identifier.
...
The final frontend-session lifecycle validation on 2026-09-11 passed:

| Command | Result |
| --- | --- |
| `node --test target/foundation/server-runtime/test/*.test.js` | PASS |
| `npm test --prefix target/foundation/ui-runtime` | PASS (no-SID response hand-off and A → B replacement) |
| `npm test --prefix target/modules/hello` | PASS |
| `node --test tests/integration/kernel/phase4-authenticated-private.test.js` | PASS |
| `scripts/test-env/kernel/phase4.4-validation.sh` | PASS (clean and `e8e7bac8e` upgrade, including a `ctime=NULL` OTAK) |
| `node --test tests/integration/kernel/ui-runtime-browser.test.js` | PASS |
| `node --test tests/integration/kernel/hello-browser-e2e.test.js` | PASS |
| `scripts/test-env/kernel/check.sh` | PASS |
