...
| HTTP session bridge | `server-core/lib/input.js::{_parseHeader,_authorization,authorization}` and `ui-essentials/socket/utils.js::makeHeaders` | `server-runtime/lib/input.js` + `SessionManager` / `ui-runtime::ServiceClient` | Validates/reuses `regsid` from `x-param-keysel: regsid` + `x-param-regsid` when cookies are unavailable | none | kernel | existing `cookie` context lookup / Yellow Page | no | no | historical alternate selectors excluded | Phase 4.4 corrective pass |
...
| `Input._parseHeader` / `Input.authorization` service branch | `RUNTIME_SESSION` / `AUTHORIZATION_BRIDGE` | JavaScript-only header-to-context recovery; it validates through the existing `cookie` context query and adds no SQL object. |
