...
function contextValue(context = {}) {
  const organisationId = Number(context.organisationId);
  const principalId = context.principalId;
  if (!Number.isInteger(organisationId) || organisationId < 1 || !/^[a-f0-9]{16}$/i.test(principalId || "")) {
    throw new MfsError("MFS_CONTEXT_INVALID", "MFS context requires an organisationId and 16-character principalId");
  }
  return { organisationId, principalId: principalId.toLowerCase() };
}
...
async function validateProvisioning({ store, database, context } = {}) {
  ...
  const expectedName = databaseName(target.principalId);
  if (!snapshot.state && !snapshot.databaseExists) return { available: false, valid: false, status: "installed", missing: ["provisioning"], conflicts: [], context: target };
  const conflicts = [];
  const missing = [];
  if (!snapshot.state) missing.push("provisioning-state");
  if (!snapshot.databaseExists) missing.push("context-database");
  if (snapshot.state && snapshot.state.database_name !== expectedName) conflicts.push("context database locator differs from the deterministic module locator");
  ...
  if (snapshot.databaseExists && snapshot.roots.length !== 1) missing.push("root-node");
  ...
  if (root && (root.owner_id !== target.principalId || root.file_path !== "/" || root.parent_id !== "0" || root.category !== "root")) conflicts.push("namespace root has incompatible historical semantics");
  const owner = snapshot.permissions.filter((entry) => entry.resource_id === "*" && entry.entity_id === target.principalId && Number(entry.permission) === 63);
  if (snapshot.databaseExists && owner.length !== 1) missing.push("root-owner-permission");
  const valid = !missing.length && !conflicts.length;
  return { available: valid, valid, status: conflicts.length ? "conflicting" : missing.length ? "partial" : "provisioned", missing, conflicts, context: target, databaseName: expectedName, rootId: root && root.id || null };
}
...
async function provision({ store, database, context } = {}) {
  ...
  const name = databaseName(target.principalId);
  await value.beginProvisioning({ ...target, databaseName: name });
  try {
    const rootId = await value.createNamespace({ ...target, databaseName: name });
    await value.finishProvisioning({ ...target, rootId });
  } catch (error) {
...
class MfsNamespace {
  ...
  async makeDirectory(parentId, name) { await this.requireAvailable(); return this.store.makeDirectory(this.context, parentId, name); }
  async resolveNode(node) { await this.requireAvailable(); return this.store.resolveNode(this.context, node); }
  async listChildren(parentId) { await this.requireAvailable(); return this.store.listChildren(this.context, parentId); }
}
