...
function database_name(principal_id) {
...
class SqlMfsStore {
  constructor({ database } = {}) {
    ...
    this.execute_script = typeof database.execute_script === "function" ? database.execute_script.bind(database) : null;
  }

  async inspect_installation() {
  ...
  async install_schemas() {
    if (!this.execute_script) throw new MfsError("MFS_SCRIPT_EXECUTOR_REQUIRED", "system-mfs installation requires a SQL script execution adapter");
    for (const filename of schema_files("install")) await this.execute_script(fs.readFileSync(filename, "utf8"), { database: "yp" });
  }

  async inspect_identity({ organisation_id, principal_id }) {
  ...
  async inspect_context({ organisation_id, principal_id }) {
    const name = database_name(principal_id);
  ...
  async begin_provisioning({ organisation_id, principal_id, database_name: name }) {
    await this.query(
      ...
      organisation_id, principal_id, name, null, read_manifest().schemaVersion
    );
  }

  async create_namespace({ principal_id, database_name: name }) {
    if (!this.execute_script) throw new MfsError("MFS_SCRIPT_EXECUTOR_REQUIRED", "system-mfs provisioning requires a SQL script execution adapter");
    for (const filename of schema_files("provision")) {
      const script = fs.readFileSync(filename, "utf8").replaceAll("{{DATABASE}}", name);
      await this.execute_script(script, { database: "yp" });
    }
    if (this.runtime_user) {
      ...
      await this.query(`GRANT SELECT,INSERT,UPDATE,DELETE,EXECUTE ON ${quote_identifier(name)}.* TO ${quote_identifier(this.runtime_user)}@'%'`);
    }
  ...
  async finish_provisioning({ organisation_id, principal_id, root_id }) {
  ...
  async fail_provisioning({ organisation_id, principal_id }, code) {
