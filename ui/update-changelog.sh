...
pkg_json="${base}/src/ui-team/package.json"
changelog="${base}/debian/changelog"
git_dir="${base}/src/ui-team"
...
entry="drumee-ui-pod (${version}) stable; urgency=medium
${bullets}
 -- ${maintainer} <${email}>  ${date_str}"

tmpfile=$(mktemp)

if grep -q "^drumee-ui-pod (${version})" "$changelog"; then
  # Find line number of the existing entry and the next entry
  start=$(grep -n "^drumee-ui-pod (${version})" "$changelog" | head -1 | cut -d: -f1)
  next_line=$(awk "NR > $start && /^drumee-ui-pod \(/ {print NR; exit}" "$changelog")
